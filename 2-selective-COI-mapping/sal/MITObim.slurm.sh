...
sample=sal
readsSE=
reads1=/home/478358/WORKING/GYRO/mt-genomes/2-selective-COI-mapping/../1-trimmed-data/sal-trimmed-min50-interleaved.fastq.gz
reads2=
...
zcat $reads1 $reads2 $readsSE | perl -ne 'chomp; $h=$_; $s=<>; $p=<>; $q=<>; if (length($s) > 70){print "$h\n$s$p$q"}' > $sample-reads.fastq
